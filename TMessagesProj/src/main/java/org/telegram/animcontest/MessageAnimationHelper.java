...
public class MessageAnimationHelper{
	private static final String TAG="MessageAnimationHelper";

	private ChatActivity chatActivity;
	private ChatActivityEnterView input;

	private int nextMessageOffsetY, inputX, inputY, inputBarX, inputBarY, inputBarH, nextMessageScrollY, msgY, inputTopViewH;
	...
	private Canvas fakeCanvas=new Canvas();
...
//		Log.w(TAG, "animateMessage() called with: cell = ["+cell+"]");
//		Log.i(TAG, "animateMessage: next type is "+nextMessageType+" cell pos "+chatListView.getChildAdapterPosition(cell));
		...
		cell.getViewTreeObserver().addOnPreDrawListener(new ViewTreeObserver.OnPreDrawListener(){
			...
			public boolean onPreDraw(){
				...
				cell.setAlpha(1f);
//				cell.setAlpha(.5f);
//				cell.getTransitionParams().animateBackgroundBoundsInner=true;
				...
				else if(cell.getMessageObject().isPhoto()){
	...
	private void animateTextMessage(ChatMessageCell cell, List<Animator> outAnimators){
		...
		cell.getTransitionParams().deltaTop=-(input.getEditField().getTotalPaddingTop()+input.getEditField().getTop())+AndroidUtilities.dp(7)+nextMessageScrollY/*-inputTopViewH*/;
		if(cell.getMessageObject().isReply())
			cell.getTransitionParams().deltaTop-=AndroidUtilities.dp(6);
//		cell.getTransitionParams().deltaBottom=input.getEditField().getTotalPaddingBottom();
		...
		bubbleShape.addUpdateListener(new ValueAnimator.AnimatorUpdateListener(){
			@Override
		...
		bubbleX.addUpdateListener(new ValueAnimator.AnimatorUpdateListener(){
			@Override
			public void onAnimationUpdate(ValueAnimator animation){
		...
		bubbleTextSize.addUpdateListener(new ValueAnimator.AnimatorUpdateListener(){
			@Override
			public void onAnimationUpdate(ValueAnimator animation){
		...
		timeAppear.addUpdateListener(new ValueAnimator.AnimatorUpdateListener(){
			@Override
			public void onAnimationUpdate(ValueAnimator animation){
	...
	private void animateAnimatedEmojiMessage(ChatMessageCell cell, List<Animator> outAnimators){
		...
		placeholderCrossfade.addUpdateListener(new ValueAnimator.AnimatorUpdateListener(){
			@Override
			public void onAnimationUpdate(ValueAnimator animation){
	...
	private void animateReplyLayout(ChatMessageCell cell, List<Animator> outAnimators){
		...
		animatorOfEverything.addUpdateListener(new ValueAnimator.AnimatorUpdateListener(){
			@Override
			public void onAnimationUpdate(ValueAnimator animation){
...
//		if  (!chatListView.fastScrollAnimationRunning && Math.abs(cell.getTranslationY()) < cell.getMeasuredHeight() * 3f) {
...
//		duration*=5;
	...
	public void beforeSendTextMessage(CharSequence message){
		Log.w(TAG, "beforeSendTextMessage() called with: message = ["+message+"]");
		updateInputBounds();
	...
	public void beforeSendStickerMessage(View view, TLRPC.Document sticker){
	...
	public void beforeSendPhotos(ArrayList<PhotoAttachPhotoCell> cells, ArrayList<SendMessagesHelper.SendingMediaInfo> photos){
	...
	public void beforeSendVoiceMessage(){
		Log.w(TAG, "beforeSendVoiceMessage() called");
		nextMessageType=MessageType.VOICE;
		updateInputBounds();
	}

	public void setupInputDownsizeAnimation(ValueAnimator anim){
		Log.w(TAG, "setupInputDownsizeAnimation() called with: anim = ["+anim+"]");
...
//		Log.w(TAG, "setupExistingMessagesSlideUpAnimation() called with: anim = ["+anim+"]");
		if(needAnimateList){
			AnimationSettings.MessageAnimationParameters params=getParamsForNextMessage();
			setupAnimator(anim, params.yPositionTiming, params.duration);
		}
	...
	private void updateInputBounds(){
		...
		inputTopViewH=input.getTopViewHeight();
