 ...
 */
public class MergeRecyclerAdapter extends RecyclerListView.SelectionAdapter{
	...
	public void addAdapter(int index, RecyclerView.Adapter adapter){
		...
		notifyItemRangeInserted(getPositionForAdapter(adapter), adapter.getItemCount());
	}

	public void removeAdapter(RecyclerView.Adapter adapter){
		int pos=getPositionForAdapter(adapter);
		if(adapters.remove(adapter)){
			adapter.unregisterAdapterDataObserver(observers.get(adapter));
			observers.remove(adapter);
			notifyItemRangeRemoved(pos, adapter.getItemCount());
		}
	}
	...
	public boolean isEnabled(RecyclerView.ViewHolder holder){
		RecyclerView.Adapter adapter=getAdapterForPosition(holder.getAdapterPosition());
		if(adapter instanceof RecyclerListView.SelectionAdapter)
			return ((RecyclerListView.SelectionAdapter) adapter).isEnabled(holder);
		return false;
	}
